...
/// Multidim Conv + Sigmoid + Mul (Attention Gate / Squeeze-Excitation)
/// INPUT:
...
///   sigmoid_out = sigmoid(conv_out)
///   output = sigmoid_out * conv_out  (element-wise multiplication)
///
...
/// ONNX Conv+Sigmoid+Mul operation - creates output tensor and performs fused operation
pub fn conv_sigmoid_mul(
    comptime T: type,
    ...
    // Calculate output shape (same as conv output)
    const output_shape = try get_conv_sigmoid_mul_output_shape(T, pkg_allocator, &input_shape, weight.shape, stride, pads, dilations, auto_pad);
    ...
    try conv_sigmoid_mul_lean(T, input_ptr, weight, &output_tensor, bias, stride, pads, dilations, group, auto_pad);

    return output_tensor;
}

/// ONNX Conv+Sigmoid+Mul operation - lean version that writes to pre-allocated output tensor
pub fn conv_sigmoid_mul_lean(
    comptime T: type,
    ...
    // Allocate temporary buffer for conv output (needed for sigmoid + mul)
    const total_elements = batch_size * out_channels * out_height * out_width;
    const conv_output = try pkg_allocator.alloc(T, total_elements);
    defer pkg_allocator.free(conv_output);
        ...
        for (0..out_channels) |m| {
            ...
            for (0..out_height) |oh| {
                for (0..out_width) |ow| {
                    ...
                    // Store convolution result in temporary buffer
                    const conv_idx = ((n * out_channels + m) * out_height + oh) * out_width + ow;
    ...
    // Apply fused Sigmoid + Mul: output = sigmoid(conv) * conv
    for (0..total_elements) |i| {
        const conv_val = conv_output[i];
        // sigmoid(x) = 1 / (1 + exp(-x))
        const sigmoid_val = 1.0 / (1.0 + @exp(-conv_val));
        // Element-wise multiply: sigmoid(conv) * conv
        output.data[i] = sigmoid_val * conv_val;
    }
}

// Compute output shape (same as convolution output)
pub inline fn get_conv_sigmoid_mul_output_shape(
    comptime T: type,
