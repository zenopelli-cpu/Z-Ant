...
test "convRelu basic functionality" {
    ...
    // Test convolution
    var result = try conv_relu(f32, &input, &weight, null, null, null, null, null, null);
    defer result.deinit();
...
test "convRelu negative basic functionality" {
    ...
    // Fill input with simple values 1..9
    for (0..9) |i| {
        input.data[i] = @as(f32, @floatFromInt(i + 1));
    }

    // Kernel 1x1x2x2 with negative values
    var weight_shape = [_]usize{ 1, 1, 2, 2 }; // [M=1, C=1, kH=2, kW=2]
    var weight = try Tensor(f32).fromShape(&pkg_allocator, &weight_shape);
    defer weight.deinit();

    // kernel generating negative values
    weight.data[0] = 1.0;
    weight.data[1] = 0.0;
    weight.data[2] = 0.0;
    weight.data[3] = -2.0;
    ...
    // Verify some values (diagonal kernel should add diagonal elements)
    try testing.expect(result.data[0] == 0); // -9 --> 0
    try testing.expect(result.data[1] == 0); // -10 --> 0
    try testing.expect(result.data[2] == 0); // -12 --> 0
    try testing.expect(result.data[3] == 0); // -13 --> 0
}
...
test "conv with stride and padding" {
    ...
    var result = try conv_relu(f32, &input, &weight, null, &stride_vals, &pad_vals, null, null, null);
    defer result.deinit();
