...
pub const Quantize = struct {
    ...
    pub fn init(comptime input_type: type, inputTensor: *Tensor(input_type), comptime output_type: type) !Quantize {
    ...
    pub fn get_output_shape(self: Quantize) []usize {
        return self.output.getShape();
    }

    pub fn get_output_tensor(self: Quantize) *TensorZant {
        return self.output;
    }
