...
while (idx_6 < 2) : (idx_6 += 1) {
                const addr_7 = @intFromPtr(input_0.ptr) + ((@as(usize,@intCast(idx_3))*2) + (@as(usize,@intCast(idx_6))*1))*@sizeOf(f32); // GEP id=7 (via VIEW)
                const addr_8 = @intFromPtr(input_1.ptr) + ((@as(usize,@intCast(idx_6))*3) + (@as(usize,@intCast(idx_4))*1))*@sizeOf(f32); // GEP id=8 (via VIEW)
    ...
    _ = &buf_10;
            ...
            const addr_13 = @intFromPtr(output_2.ptr) + ((@as(usize,@intCast(idx_3))*3) + (@as(usize,@intCast(idx_4))*1))*@sizeOf(f32); // GEP id=13 (Buffer)
